            ...
            if(Aid==rec['libid']):
                ...
                if(p==1):
                    ...
                    if(r):
                        ...
                        print("No data found for the name of book entered.")
                    ...
                    q="delete from book where name ='{0}'".format(book)
                    smt.execute(q)
                    conn.commit()
            else:
                    ...
                    if(ch==1):                  
                        ...
                        if(r):
                            if(r['name']==book):
                                r['quantity']+=1
                                qt=r['quantity']
                                q="update book set quantity ={0} where name='{1}'".format(qt,book)
                                smt.execute(q)
                                conn.commit()
                                print("Book Returned succesfully") 
                        else:
                            print("There was a problem returning the book, please try again....")                  
                        ...
                        if(records):
                            records['quantity']-=1
                            qu=records['quantity']
                            q="update book set quantity ={0} where name='{1}'".format(qu,book)
                            smt.execute(q)
                            conn.commit()
